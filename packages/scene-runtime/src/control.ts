 ...
 *
 * Controls are scene-global rather than per-entity for the reason given in
 * `SceneRuntimeOptions`: there is one camera, and two characters sharing a view
 * must not disagree about which way forward is.
 */

export interface SceneControlKeyframe {
  tick: number;
  cameraYawRad: number;
}

/** The scene-global control values one tick runs with. */
export interface SceneControlState {
  cameraYawRad: number;
}

/**
 * Sampled by the scene before each tick.
 *
 ...
 */
export interface SceneControlSource {
  sample(tick: number): SceneControlState;
}
 ...
 */
export class RecordedControlSource implements SceneControlSource {
  private readonly frames: SceneControlKeyframe[];

  constructor(keyframes: readonly SceneControlKeyframe[]) {
...
export function validateControlTrack(
  keyframes: readonly SceneControlKeyframe[],
  tickCount: number,
