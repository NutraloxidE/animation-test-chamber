...
beforeEach(() => {
  ...
  cpSync(join(SOURCE_REPO, "projects"), join(repoRoot, "projects"), {
    recursive: true,
  });
  cpSync(
    join(SOURCE_REPO, "assets/prefabs"),
    join(repoRoot, "assets/prefabs"),
    {
      recursive: true,
    },
  );
  cpSync(
    join(SOURCE_REPO, "assets/animation"),
    join(repoRoot, "assets/animation"),
    {
      recursive: true,
    },
  );
...
function animatorAssignment() {
  const resolved = resolveGameObjectPrefab(
    registry(),
    registry().referenceTo("navigator", "1.0.0"),
  ).prefab;
  ...
  if (animator?.componentType !== "animator")
    throw new Error("navigator fixture has no Animator");
...
function fixtureRoot(
  assignment: ReturnType<typeof animatorAssignment>,
  slot: AssignmentSlot,
): PrefabNodeDefinition {
  const unrelatedAssignment = {
    ...
    instanceOverrides: [
      { path: "/graph/states/walk/speed", op: "set" as const, value: 9 },
    ],
  };
  return {
    nodeId: "root",
    ...
    children: [
      {
        ...
        node: {
          ...
          components: [
            {
              schemaVersion: 1,
              componentId: "unrelated-animator",
              componentType: "animator",
              enabled: true,
              assignment: unrelatedAssignment,
            },
            {
              schemaVersion: 1,
              componentId: "target-animator",
              componentType: "animator",
              enabled: true,
              assignment,
            },
...
function storePrefab(document: GameObjectPrefabAsset): void {
  const path = join(
    repoRoot,
    prefabAssetFilePath(document.metadata.id, document.metadata.version),
  );
  mkdirSync(resolve(path, ".."), { recursive: true });
  writeFileSync(path, `${JSON.stringify(document, null, 2)}\n`);
}

function addMatrixFixture(
  derivation: Derivation,
  slot: AssignmentSlot,
): {
  targetId: string;
  source: AssetReference;
  oldTarget: GameObjectPrefabAsset;
  parentPath?: string;
} {
  ...
  if (derivation === "fork") {
    ...
    const document = sealAsset({
      ...
      derivation: {
        mode: "fork",
        forkedFrom: origin,
        forkIntent: "Adoption matrix fixture.",
      },
      abstract: false,
      root,
    }) as GameObjectPrefabAsset;
  ...
  const parent = sealAsset({
    metadata: fixtureMetadata(parentId),
    derivation: { mode: "base" },
    abstract: true,
    root,
  }) as GameObjectPrefabAsset;
      ...
      patches: [
        {
          ...
          nodeId: "animated-child",
          componentId: "target-animator",
          patches: [{ path: "/enabled", op: "set", value: true }],
        },
        {
          kind: "patch-component",
          nodeId: "animated-child",
          componentId: "target-animator",
          patches: [{ path: `/assignment/${slot}`, op: "set", value: source }],
        },
      ],
    },
    abstract: false,
  ...
  return {
    ...
    source,
    oldTarget: document,
    parentPath: prefabAssetFilePath(parentId, "1.0.0"),
  };
}
...
function exactAnimationRequest(source: AssetReference, targetId: string) {
  ...
  return {
    ...
    draft: {
      ...
      metadata: {
        ...sourceAsset.metadata,
        description: `${sourceAsset.metadata.description} Matrix publication.`,
      },
    } as AnimationAsset,
    expected: {
      sourceContentHash: source.contentHash,
...
function prefabAdoptionRequest(
  targets: Array<{ sceneId: string; gameObjectId: string }>,
) {
  const source = registry().referenceTo("gameplay-quaternius", "1.0.0");
...
function repositoryDigest(): string {
  ...
  for (const root of ["assets", "generated", "projects"]) {
    for (const entry of readdirSync(join(repoRoot, root), { recursive: true })
      .map(String)
      .sort()) {
...
describe("Prefab API exact identity", () => {
  it("lists and reads an exact stored Prefab version", async () => {
    ...
    const response = await app.request(
      `/api/prefabs/${reference.assetId}/${reference.version}`,
    );
  ...
  it("returns 404 for an unknown exact version and 409 for a pinned hash mismatch", async () => {
    const reference = registry().referenceTo(
      registry().all()[0]!.id,
      registry().all()[0]!.version,
    );
    const app = api();

    expect(
      (await app.request(`/api/prefabs/${reference.assetId}/999.0.0`)).status,
    ).toBe(404);
    expect(
      (
        await app.request(
  ...
  it("reports exact usage and delete blockers", async () => {
    const reference = registry().referenceTo("gameplay-quaternius", "1.0.0");
    const app = api();
...
describe("Prefab API immutable publication", () => {
  it("publishes a new version transactionally and refuses overwriting it", async () => {
    const stored = registry()
      .all()
      .find((entry) => entry.document.derivation.mode === "base")!;
    ...
    const path = join(
      repoRoot,
      prefabAssetFilePath(published.assetId, published.version),
    );
  ...
  it("publishes variants and forks without mutating their source lineage", async () => {
    ...
    const variant = sourceRegistry.get(
      sourceRegistry.referenceTo("navigator", "1.0.0"),
    );
    ...
    if (!prefabHasStoredRoot(sourceDocument))
      throw new Error("camera fixture must store a root");
    ...
    expect(
      ...
      JSON.stringify(await variantResponse.clone().json()),
    ).toBe(200);
    ...
    expect(
      forkResponse.status,
      JSON.stringify(await forkResponse.clone().json()),
    ).toBe(200);
    expect(
      readFileSync(
        join(repoRoot, prefabAssetFilePath(source.assetId, source.version)),
    ...
    expect(
      registry().referenceTo(fork.metadata.id, fork.metadata.version),
    ).toMatchObject({
...
describe("Prefab to Scene exact-target adoption", () => {
  it("publishes only without writing the Project or moving any holder", async () => {
    ...
    const response = await api().request(
      "/api/prefabs/publish-and-adopt-scenes",
      {
        method: "POST",
        headers: { "content-type": "application/json" },
    ...
    expect(responseBody.changedPaths).not.toContain(
  ...
  it("moves exactly one named Scene instance and bumps the Project revision", async () => {
    ...
    const untouchedBefore = JSON.stringify(
      beforeScene.gameObjects?.find(
        (gameObject) => gameObject.id === "scripted-humanoid",
      ),
    );
    const response = await api().request(
      "/api/prefabs/publish-and-adopt-scenes",
    ...
    const afterScene = after.scenes.find(
      (scene) => scene.id === target.sceneId,
    )!;
    expect(
      afterScene.gameObjects?.find(
        (gameObject) => gameObject.id === target.gameObjectId,
      )?.prefab,
    ).toMatchObject({ assetId: "gameplay-quaternius", version: "1.0.1" });
    expect(
      JSON.stringify(
        afterScene.gameObjects?.find(
          (gameObject) => gameObject.id === "scripted-humanoid",
  ...
  it.each([
    [
      "stale revision",
      (request: ReturnType<typeof prefabAdoptionRequest>) => {
    ...
    [
      ...
      (request: ReturnType<typeof prefabAdoptionRequest>) => {
        request.targets = [
          { sceneId: "missing-scene", gameObjectId: "missing-object" },
        ];
      },
    ],
    ...
    expect((await body(response)).changedTargets).toEqual({
      sceneInstances: [],
    });
    expect(
      readFileSync(
    ...
    expect(
      readdirSync(join(repoRoot, "assets/prefabs"), { recursive: true }).map(
        String,
      ),
...
describe("Animation to Prefab exact-target adoption", () => {
  it.each([
    ...
    "adopts %s × %s through stored files and the real resolver",
    async (derivation, slot) => {
      ...
      const oldAnimationPath = join(
        repoRoot,
        assetFilePath(
          fixture.source.assetType,
          fixture.source.assetId,
          fixture.source.version,
        ),
      );
      const oldAnimationBytes = readFileSync(oldAnimationPath, "utf8");
      const oldPrefabPath = join(
        repoRoot,
        prefabAssetFilePath(fixture.targetId, "1.0.0"),
      );
      const oldPrefabBytes = readFileSync(oldPrefabPath, "utf8");
      const parentBytes = fixture.parentPath
        ? readFileSync(join(repoRoot, fixture.parentPath), "utf8")
        : undefined;
      const nonTargets = new Map(
        registry()
          .all()
          .filter((entry) => entry.id !== fixture.targetId)
          .map((entry) => {
      ...
      const response = await api().request(
        "/api/animation-assets/publish-and-adopt-prefabs",
        {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify(request),
      ...
      const responseBody = await body(response);
      expect(response.status, JSON.stringify(responseBody)).toBe(200);
      expect(responseBody.changedTargets).toEqual({
        prefabIds: [fixture.targetId],
      });
      const published = responseBody.published as {
      ...
      expect(
        readFileSync(
          join(
            repoRoot,
            assetFilePath(
              published.animation.assetType,
              published.animation.assetId,
              published.animation.version,
            ),
          ),
          "utf8",
        ),
      ).toContain(published.animation.contentHash);

      const nextRegistry = registry();
      const nextReference = nextRegistry.referenceTo(
        publishedPrefab.assetId,
        publishedPrefab.version,
      );
      ...
      const targetAnimator = walkResolvedNodes(resolved.root)
        .find((node) => node.nodeId === "animated-child")
        ?.components.find(
          (component) => component.componentId === "target-animator",
        );
      expect(targetAnimator?.componentType).toBe("animator");
      if (targetAnimator?.componentType !== "animator")
        throw new Error("target Animator did not resolve");
      ...
      for (const other of ["behavior", "motionSet", "rig", "tuning"] as const) {
        if (other !== slot)
          expect(targetAnimator.assignment[other]).toEqual(
            animatorAssignment()[other],
          );
      }
      const unrelated = walkResolvedNodes(resolved.root)
        .find((node) => node.nodeId === "animated-child")
        ?.components.find(
          (component) => component.componentId === "unrelated-animator",
        );
      expect(
        unrelated?.componentType === "animator"
          ? unrelated.assignment[slot]
          : undefined,
      ).toEqual({ ...fixture.source, contentHash: "f".repeat(64) });
      ...
      if (fixture.parentPath)
        expect(readFileSync(join(repoRoot, fixture.parentPath), "utf8")).toBe(
          parentBytes,
        );
      for (const [path, bytes] of nonTargets)
        expect(readFileSync(join(repoRoot, path), "utf8"), path).toBe(bytes);
      expect(
        readFileSync(
          join(repoRoot, "generated/animation-assets/library-index.json"),
          "utf8",
        ),
      ).toContain(published.animation.contentHash);
      expect(
      ...
      ).toContain(publishedPrefab.contentHash);
      const stored = JSON.parse(
        readFileSync(
          join(
            repoRoot,
            prefabAssetFilePath(fixture.targetId, publishedPrefab.version),
          ),
          "utf8",
        ),
      ) as GameObjectPrefabAsset;
      ...
      if (derivation === "variant") {
        ...
        if (stored.derivation.mode !== "variant")
          throw new Error("matrix variant changed derivation");
        const terminal = stored.derivation.patches.filter(
          (patch) =>
            patch.kind === "patch-component" &&
            patch.nodeId === "animated-child" &&
            patch.componentId === "target-animator" &&
            patch.patches.some((entry) => entry.path === `/assignment/${slot}`),
        );
  ...
  it("publishes a real Animator reference change in exactly the named Prefab", async () => {
    ...
    if (animator?.componentType !== "animator")
      throw new Error("navigator must resolve an Animator");
  ...
  it("refuses a stale holder snapshot with zero animation or Prefab writes", async () => {
    ...
    const animationBefore = readdirSync(join(repoRoot, "assets/animation"), {
      recursive: true,
    }).map(String);
    const prefabsBefore = readdirSync(join(repoRoot, "assets/prefabs"), {
      recursive: true,
    }).map(String);
    ...
    expect(
      readdirSync(join(repoRoot, "assets/animation"), { recursive: true }).map(
        String,
      ),
