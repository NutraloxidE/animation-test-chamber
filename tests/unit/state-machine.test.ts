...
const stateOf = (id: string) =>
  project.graph.states.find((state) => state.id === id);
...
describe("transition conditions", () => {
  ...
  beforeEach(() => {
    graph = new AnimationGraphRuntime(
      project.graph,
      motionResolverFor(project),
    );
  });
...
describe("cancel windows", () => {
  ...
  it("leaves guard for another action without releasing guard first", () => {
    ...
    graph.tick(
      makeParams({ booleans: { guardHeld: true }, buffered: { Dodge: true } }),
    );
    expect(graph.getLayer("action").stateId).toBe("dodge");
  });
});
...
describe("state lifecycle", () => {
  it("falls back to the declared state when a one-shot clip finishes", () => {
    ...
    for (let i = 0; i < Math.round(0.8 / FIXED_DT); i += 1)
      graph.tick(makeParams({}));
    expect(graph.getLayer("action").stateId).toBe("action-none");
  });
  ...
  it("blends in over the transition duration rather than snapping", () => {
    ...
    const previousAtTransition = layer.previousNormalizedTime;

    const unchanged = structuredClone(graph.getLayer("locomotion"));
    expect(graph.getLayer("locomotion")).toEqual(unchanged);

    graph.tick(makeParams({ numbers: { moveMagnitude: 0.3 } }));
    expect(graph.getLayer("locomotion").previousNormalizedTime).toBeGreaterThan(
      previousAtTransition,
    );

    for (let i = 0; i < Math.round(0.18 / FIXED_DT); i += 1) {
      graph.tick(makeParams({ numbers: { moveMagnitude: 0.3 } }));
    }
    expect(graph.getLayer("locomotion").blendWeight).toBe(1);
  });

  it("replays both crossfade clocks and weight deterministically", () => {
    const run = () => {
      const graph = new AnimationGraphRuntime(
        project.graph,
        motionResolverFor(project),
...
describe("semantic events", () => {
  ...
  it("emits looping clip events once per cycle", () => {
    ...
    for (let i = 0; i < twoCycles; i += 1) {
      ...
      contacts += result.events.filter(
        (event) => event.kind === "FootContactLeft",
      ).length;
    }
...
describe("live graph updates", () => {
  it("keeps playing the current state when canonical data is swapped in", () => {
    ...
    const transition = edited.graph.transitions.find(
      (t) => t.id === "idle-to-walk",
    )!;
...
describe("playback speed", () => {
  it("multiplies the state speed into the arriving transition speed", () => {
    ...
    expect(graph.getLayer("locomotion").stateId).toBe("walk");
    expect(graph.getLayer("locomotion").playbackSpeed).toBeCloseTo(
      walk.playbackSpeed * 2,
    );
  });
...
describe("equipment", () => {
  ...
  it("keeps the two guard entries mutually exclusive", () => {
    for (const transition of project.graph.transitions) {
      if (transition.to !== "guard" && transition.to !== "guard-shield")
        continue;
      const condition = transition.conditions.find(
        (c) => c.parameter === "equippedShield",
      );
      expect(
        condition,
        `${transition.id} must test equippedShield`,
      ).toBeDefined();
  ...
  it("resolves the declared parameter from equipment state, not from code", () => {
    const guardAfterEquipping = (equipShield: boolean): string => {
      ...
      for (
        let tick = 1;
        tick < 20 && record.actionState === "action-none";
        tick += 1
      ) {
...
describe("weapon modes", () => {
  ...
  it("applies a transition timing override for one weapon only", () => {
    const edited = structuredClone(project);
    const combo = edited.graph.transitions.find(
      (t) => t.id === "attack-01-to-attack-02",
    )!;
    const shared = combo.cancelWindow;
    combo.weaponOverrides = {
      magic: { cancelWindow: { start: 0.5, end: 0.9 } },
    };
  ...
  it("gives every catalog weapon mode its own attack clips", () => {
    const attackStates = project.graph.states.filter((state) =>
      WEAPON_MODES.some(
        (mode) =>
          clipIdForState(project, state.id, mode.id) !==
          clipIdForState(project, state.id),
      ),
    );
    expect(attackStates.length).toBeGreaterThan(0);

    for (const state of attackStates) {
      const clipIds = WEAPON_MODES.map((mode) =>
        clipIdForState(project, state.id, mode.id),
      );
